...
const StoryMap: React.FC<StoryMapProps> = ({ 
  ...
  // Create edges for each plot thread
  const newEdges: Edge[] = []
  plotThreads.forEach((thread) => {
    const threadScenes = sortedScenes.filter((scene) => 
      scene.plotThreads.includes(thread.id)
    )
    
    // Create edges between consecutive scenes in this thread
