...
export function SceneEditor({
  ...
  const handleBeatUpdate = (sceneId: string, beatId: string) => {
    const scene = scenes.find((s) => s.id === sceneId)
    if (scene && selectedStructure) {
      ...
      if (beat) {
        handleSceneUpdate({
          ...
          plotThreads: [...new Set([...scene.plotThreads, ...beat.requiredThreads])],
              ...
              .map((scene) => (
                <TableRow key={scene.id}>
                  ...
                  <TableCell>
                    <Select
                      value=""
                      onValueChange={(value: string) => handleBeatUpdate(scene.id, value)}
